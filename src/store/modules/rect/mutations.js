...
export default {
    ...
    [ADD_THE_LISTS](state, payload) {
        state.rects.push(payload.A_List);
        console.log(JSON.stringify(state.rects));
        const lefts = state.rects.map((d)=>d.left);
        const tops = state.rects.map((d)=>d.top);
        console.log('----top:', tops);
        console.log('----left:', lefts);
    },
    
    [REMOVE_THE_LIST](state, id) {
